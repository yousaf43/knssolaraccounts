...
import { Building2, CalendarDays, Check, Edit3, Loader2, Plus, ShieldCheck, Users, X } from "lucide-react";
...
export default function PlatformAdmin() {
  ...
  const openEdit = (company: Company) => {
    setEditing(company);
    setForm({ name: company.name, contact_email: company.contact_email || "", phone: company.phone || "", address: company.address || "", plan: company.plan, status: company.status, starts_at: company.starts_at, expires_at: company.expires_at || "", notes: company.notes || "" });
    setFormOpen(true);
  };
  const saveCompany = async () => {
    if (!form.name.trim()) { toast.error("Company name is required"); return; }
    setSaving(true);
    const payload = { ...form, name: form.name.trim(), contact_email: form.contact_email || null, phone: form.phone || null, address: form.address || null, expires_at: form.expires_at || null, notes: form.notes || null };
    const result = editing
      ? await supabase.from("companies").update(payload).eq("id", editing.id)
      : await supabase.from("companies").insert(payload);
    if (result.error) toast.error(result.error.message);
    else { toast.success(editing ? "Company updated" : "Company created"); setFormOpen(false); await load(); }
    setSaving(false);
  };
  ...
  return <div className="space-y-6 p-1">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
      <div><p className="text-sm font-medium text-primary">Platform control</p><h1 className="text-3xl font-semibold tracking-tight">Companies</h1><p className="mt-1 text-sm text-muted-foreground">Manage customer workspaces, plans, access, and usage.</p></div>
      <Button onClick={openCreate}><Plus className="mr-2 h-4 w-4" /> Add company</Button>
    </div>
    <div className="grid gap-4 sm:grid-cols-3">
      <Card><CardContent className="flex items-center gap-4 p-5"><Building2 className="h-8 w-8 text-primary" /><div><p className="text-2xl font-semibold">{companies.length}</p><p className="text-xs text-muted-foreground">Total companies</p></div></CardContent></Card>
      <Card><CardContent className="flex items-center gap-4 p-5"><Check className="h-8 w-8 text-emerald-600" /><div><p className="text-2xl font-semibold">{activeCount}</p><p className="text-xs text-muted-foreground">Active workspaces</p></div></CardContent></Card>
      <Card><CardContent className="flex items-center gap-4 p-5"><Users className="h-8 w-8 text-sky-600" /><div><p className="text-2xl font-semibold">{Object.values(stats).reduce((sum, item) => sum + item.users, 0)}</p><p className="text-xs text-muted-foreground">Team members</p></div></CardContent></Card>
    </div>
