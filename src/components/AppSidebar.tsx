...
export function AppSidebar({ onNavigate }: AppSidebarProps) {
  ...
  return (
    <aside
      className={`app-sidebar relative flex flex-col bg-sidebar text-sidebar-foreground border-r border-sidebar-border lg:rounded-l-2xl transition-[width] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] ${
        isCollapsed ? "w-16" : "w-64"
      } ${isMobile ? "h-full" : "h-full"} min-h-0 overflow-hidden`}
    >
      <div className="relative flex items-center justify-center px-2 py-4">
        <div className={`${isCollapsed ? "w-12" : "w-full"} rounded-lg bg-card p-2 shadow-[0_8px_24px_-16px_hsl(var(--sidebar-primary))] ring-1 ring-sidebar-primary/20 transition-all duration-300`}>
          <img
            src={settings.logoUrl || ksLogo}
      ...
      {!isCollapsed && (
        <div className="px-3 pb-3">
          <div className="flex items-center gap-2 rounded-lg border border-sidebar-border bg-sidebar-accent/50 px-3 py-2 transition-all focus-within:border-sidebar-primary/60 focus-within:shadow-[0_0_0_3px_hsl(var(--sidebar-primary)/0.10)]">
      ...
      <nav className="flex-1 min-h-0 space-y-1 overflow-y-auto px-2 pb-4">
        ...
        {displayedNavItems.map((item) => (
          <NavLink
            ...
            className="group relative flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium text-sidebar-foreground transition-all duration-200 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground hover:translate-x-0.5"
            activeClassName="bg-sidebar-accent text-sidebar-primary shadow-[inset_3px_0_0_0_hsl(var(--sidebar-primary)),0_10px_24px_-18px_hsl(var(--sidebar-primary))]"
            onClick={onNavigate}
          >
      ...
      <div className="border-t border-sidebar-border/70 p-2 space-y-1">
        ...
        {!isMobile && (
          <button
            ...
            className="flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground transition-all w-full press"
          >
            {collapsed ? (
        ...
        {!isCollapsed && (
          <NavLink
            to="/settings"
            onClick={onNavigate}
            className="mt-1 flex items-center gap-3 rounded-lg border border-sidebar-border bg-sidebar-accent/40 px-3 py-2 transition-all hover:border-sidebar-primary/30 hover:bg-sidebar-accent"
          >
