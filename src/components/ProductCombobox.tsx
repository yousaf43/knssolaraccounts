...
export function ProductCombobox({ inventory, selectedItemId, onSelect }: Props) {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-[520px] min-w-full max-h-[320px] overflow-y-auto bg-popover border rounded-md shadow-md">
          {filtered.length === 0 ? (
            ...
            filtered.map((inv) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "w-full text-left px-3 py-2 text-xs hover:bg-accent flex items-start justify-between gap-2 transition-colors",
                    selectedItemId === inv.id && "bg-accent"
                  )}
                >
                  <span className="flex flex-col min-w-0 flex-1 gap-0.5">
                    <span className="font-medium break-words whitespace-normal leading-snug">
                      <HighlightText text={inv.name} query={search} />
                    </span>
                    <span className="flex flex-wrap items-center gap-1.5 text-muted-foreground">
                      <span>(<HighlightText text={inv.sku} query={search} />)</span>
                      {inv.model && (
